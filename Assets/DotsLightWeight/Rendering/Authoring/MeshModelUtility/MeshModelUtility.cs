...
namespace DotsLite.Geometry
{
    ...
    public static class MeshModelUtility2
    {
        ...
        //public static void BuildModelToDictionary2<T>(this IEnumerable<T> models, GameObjectConversionSystem gcs)
        //    where T : ModelGroupAuthoring.ModelAuthoringBase
        //{
        ...
        //    meshmodels.CreateModelToDictionary2(meshDict, atlasDict);

        //    meshmodels.CreateModelEntitiesToDictionary(gcs, meshDict, atlasDict);
        //}

        public static void BuildModelToDictionary2(this IEnumerable<MeshModel<UI32, PositionNormalUvVertex>> models, GameObjectConversionSystem gcs)
        {
            var meshDict = gcs.GetMeshDictionary();
            var atlasDict = gcs.GetTextureAtlasDictionary();
        ...
        public static void CreateModelToDictionary2(
            this IEnumerable<MeshModel<UI32, PositionNormalUvVertex>> models,
            Dictionary<SourcePrefabKeyUnit, Mesh> meshDict, TextureAtlasDictionary.Data atlasDict)
        {
            var qMmts =
                from model in models
            ...
            var qOfs =
                ...
                let model = x.src1
                where !meshDict.ContainsKey(model.SourcePrefabKey)
                select model.BuildMeshCombiner2(meshsrc, meshDict, atlasDict);
            var ofs = qOfs.ToArray();

            var qKey = ofs.Select(x => x.key);
            var qMesh = ofs.Select(x => x.f.ToTask())
                .WhenAll().Result
                .Select(x => x.CreateMesh2());
            //var qMesh = ofs.Select(x => x.f().CreateMesh());
            meshDict.AddRange(qKey, qMesh);
        }
        ...
        //public static void CreateModelEntitiesToDictionary2(
        //    this IEnumerable<IMeshModel> models,
        //    GameObjectConversionSystem gcs,
        //    Dictionary<SourcePrefabKeyUnit, Mesh> meshDict, TextureAtlasDictionary.Data atlasDict)
        //{
