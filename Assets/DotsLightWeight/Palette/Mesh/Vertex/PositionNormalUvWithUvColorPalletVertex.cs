...
namespace DotsLite.Geometry
{
    ...
    public struct PositionUvNormalWithUvColorPaletteVertex : IVertexUnit
    {
    ...
    public class PositionUvNormalWithUvColorPaletteVertexBuilder : IVertexBuilder//, ISetBufferParams
    {
            ...
            IEnumerable<SrcMeshUnit> srcmeshes, AdditionalParameters p, Mesh.MeshData dstmesh)
        {
                ...
                IEnumerable<SrcMeshUnit> srcmeshes, AdditionalParameters p)
            {
                ...
                var uvs = srcmeshes.QueryConvertUvs(p, channel: 0).ToArray();
                var cids = srcmeshes.QueryColorPaletteSubIndex(p).ToArray();
                var uids = srcmeshes.QueryUvOffsetSubIndex(p).ToArray();
                var qVtx =
                    from x in (poss, nms, uvs, cids, uids).Zip()
